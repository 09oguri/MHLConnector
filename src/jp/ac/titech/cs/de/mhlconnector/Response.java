...
public class Response {
	...
	public byte getState() {
		if (receive[0] == 0x02 && receive[1] == 0x01) {
			switch (receive[2]) {
			case 0x50: // スタートコマンド
			case 0x51: // ストップコマンド
	...
	public long getNumOfData() {
		...
		// データ転送コマンドかどうか確認
		if (receive[0] == 0x02 && receive[1] == 0x01 && receive[2] == 0x55) {
			String tmp = "";
			for (int i = dataNumStart; i < dataNumStart + dataNumSize; i++) {
				tmp += String.format("%02x", receive[i]);
			}
	...
	public double getVolt(int unit, int ch) {
		...
		// データ転送コマンドかどうか確認
		if (receive[0] == 0x01 && receive[1] == 0x00 && receive[2] == 0x01) {
			String tmp = "";
